  ...
  const payload = {
    ...
    reply_markup: {
      inline_keyboard: [
        [
          {
            text: '📊 View Program Rankings',
            callback_data: '/sub-programs_ranking',
          },
        ],
        [
          {
            text: '💰 Check Total Value Locked (TVL)',
            callback_data: '/sub-programs_tvl',
          },
        ],
        [
          {
            text: '📈 Transactions Data',
            callback_data: '/sub-programs_transactions',
          },
        ],
        [
          {
            text: '🔄 Instructions Data',
            callback_data: '/sub-programs_instructions',
          },
        ],
        [
          {
        ...
        [
          {
            text: '🔎 Find Program Active Users',
        ...
        [
          {
            ...
            callback_data: '/sub-programs_details',
          },
        ],
...
export async function initializeProgramDefaults(chatId: number) {
  ...
  if (!existingTransactionsSettings) {
    ...
    await redis.set(
      transactionsKey,
      JSON.stringify(defaultTransactionsSettings),
    );
  }
  ...
  if (!existingInstructionsSettings) {
    ...
    await redis.set(
      instructionsKey,
      JSON.stringify(defaultInstructionsSettings),
    );
  }
  ...
  if (!existingFindActiveUsersSettings) {
    ...
    await redis.set(
      findActiveUsersKey,
      JSON.stringify(defaultFindActiveUsersSettings),
    );
  }
}
...
export async function initializeRankingFlow(chatId: number, messageId: number) {
  ...
  const settings = JSON.parse(
    (await redis.get(`${PROGRAM_SETTINGS_KEY}:${chatId}`)) || '{}',
  );
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [
          {
            text: '⏱️ Set Interval',
            callback_data: '/sub-programs_set_interval',
          },
        ],
...
export async function initializeTvlFlow(chatId: number, messageId: number) {
  const redis = RedisService.getInstance();
  const settings = JSON.parse(
    (await redis.get(`${TVL_SETTINGS_KEY}:${chatId}`)) || '{}',
  );
    ...
    reply_markup: {
      inline_keyboard: [
        [
          {
            text: '⏱️ Resolution',
            callback_data: '/sub-programs_tvl_resolution',
          },
        ],
...
export async function updateTvlResolution(chatId: number, resolution: string) {
  ...
  await redis.set(
    key,
    JSON.stringify({ ...currentSettings, resolution: `${numResolution}d` }),
  );
 ...
 */
export async function initializeTransactionsFlow(
  chatId: number,
  messageId: number,
) {
  const redis = RedisService.getInstance();
  const settings = JSON.parse(
    (await redis.get(`${TRANSACTIONS_SETTINGS_KEY}:${chatId}`)) || '{}',
  );
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [
          {
            text: '🔄 Fetch Data',
            callback_data: '/sub-programs_transactions_fetch',
          },
        ],
        [{ text: '🔙 Back', callback_data: '/programs' }],
      ],
 ...
 */
export async function updateTransactionsRange(
  chatId: number,
  messageId: number,
  range: string,
) {
...
export async function fetchTransactionsData(chatId: number, programId: string) {
  try {
    ...
    const response = await makeVybeRequest(
      `program/${programId}/transactions-count-ts?range=${range}`,
    );
    ...
    formData.append(
      'caption',
      `📊 Transactions Data (${range}) for Program ID: ${programId}`,
    );
...
export async function fetchTvlData(chatId: number, programId: string) {
  try {
    ...
    // Fetch TVL data
    const response = await makeVybeRequest(
      `program/${programId}/tvl?resolution=${resolution}`,
    );
    ...
    // Format the TVL data
    const formattedTvl = response.data
      .map(
        (entry: any) =>
      ...
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: '🔄 Fetch Again',
              callback_data: '/sub-programs_tvl_fetch',
            },
          ],
...
export async function fetchRankings(chatId: number, messageId: number) {
  try {
    ...
    if (settings.limit > 10) {
      ...
      // Fetch data
      const response = await makeVybeRequest(
        `program/ranking?limit=${settings.limit}&interval=${settings.interval}`,
      );
      if (!response || !response.data || response.data.length === 0) {
      ...
      formData.append(
        'document',
        new Blob([txtContent], { type: 'text/plain' }),
        'program_rankings.txt',
      );
      formData.append(
        'caption',
        `📊 Program Rankings (${settings.interval}) - Top ${settings.limit}`,
      );
      ...
      const formattedRankings = response.data
        .map(
          (program: any) =>
            `🏆 Rank: ${program.programRank}\n` +
            `📛 Name: ${program.programName || 'N/A'}\n` +
