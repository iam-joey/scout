 ...
 */
export async function initializeInstructionsFlow(
  chatId: number,
  messageId: number,
) {
  const redis = RedisService.getInstance();
  const settings = JSON.parse(
    (await redis.get(`${INSTRUCTIONS_SETTINGS_KEY}:${chatId}`)) || '{}',
  );
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [
          {
            text: '⚙️ Change Range',
            callback_data: '/sub-programs_instructions_range',
          },
        ],
        [
          {
            text: '🔄 Fetch Data',
            callback_data: '/sub-programs_instructions_fetch',
          },
        ],
 ...
 */
export async function updateInstructionsType(
  chatId: number,
  messageId: number,
  type: string,
) {
...
export async function updateInstructionsRange(chatId: number, range: string) {
  try {
    ...
    // Save the range
    await redis.set(
      `${INSTRUCTIONS_SETTINGS_KEY}:${chatId}`,
      JSON.stringify({
        ...settings,
...
export async function fetchInstructionsData(chatId: number, programId: string) {
  try {
    ...
    const response = await makeVybeRequest(
      `program/${programId}/instructions-count-ts?range=${range}`,
    );
    ...
    formData.append(
      ...
      'instructions_data.txt',
    );
    formData.append(
      'caption',
      `🔄 Instructions Data (${range}) for Program ID: ${programId}`,
    );
