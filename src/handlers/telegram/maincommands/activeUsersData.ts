 ...
 */
export async function initializeActiveUsersFlow(chatId: number, messageId: number) {
  const redis = RedisService.getInstance();
  const settings = JSON.parse(await redis.get(`${ACTIVEUSERS_SETTINGS_KEY}:${chatId}`) || '{}');
  ...
  await updateMessage(TELEGRAM_BASE_URL, {
    ...
    reply_markup: {
      inline_keyboard: [
        typeButtons,
        [{ text: '⚙️ Change Range', callback_data: '/sub-programs_activeusers_range' }],
        [{ text: '🔄 Fetch Data', callback_data: '/sub-programs_activeusers_fetch' }],
 ...
 */
export async function updateActiveUsersType(chatId: number, messageId: number, type: string) {
  const redis = RedisService.getInstance();
...
export async function promptActiveUsersRange(chatId: number) {
  ...
  await redis.set(`activeusers_state:${chatId}`, 'waiting_for_range', REDIS_TTL);
...
export async function updateActiveUsersRange(chatId: number, range: string) {
  try {
    ...
    // Save the range
    await redis.set(`${ACTIVEUSERS_SETTINGS_KEY}:${chatId}`, JSON.stringify({
      ...settings,
    ...
    await sendMessage(TELEGRAM_BASE_URL, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔙 Back to Settings', callback_data: '/sub-programs_activeusers' }]
        ]
      }
    });
  } catch (error) {
    ...
    await sendMessage(TELEGRAM_BASE_URL, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔄 Try Again', callback_data: '/sub-programs_activeusers_range' }],
...
export async function promptProgramIdForActiveUsers(chatId: number) {
  const redis = RedisService.getInstance();
  await redis.set(`activeusers_state:${chatId}`, 'waiting_for_program_id', REDIS_TTL);
...
export async function fetchActiveUsersData(chatId: number, programId: string) {
  try {
    ...
    const response = await makeVybeRequest(`program/${programId}/active-users-ts?range=${range}`);
    ...
    formData.append('document', new Blob([txtContent], { type: 'text/plain' }), 'active_users_data.txt');
    formData.append('caption', `👥 Active Users Data (${range}) for Program ID: ${programId}`);
    ...
    await sendMessage(TELEGRAM_BASE_URL, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔄 Fetch Again', callback_data: '/sub-programs_activeusers_fetch' }],
          [{ text: '⚙️ Change Settings', callback_data: '/sub-programs_activeusers' }],
    ...
    await sendMessage(TELEGRAM_BASE_URL, {
      ...
      reply_markup: {
        inline_keyboard: [
          [{ text: '🔄 Try Again', callback_data: '/sub-programs_activeusers_fetch' }],
          [{ text: '🔙 Back', callback_data: '/sub-programs_activeusers' }]
        ]
      }
    });
